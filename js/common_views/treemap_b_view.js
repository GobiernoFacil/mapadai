...
define(function(require){
  ...
  var treeMap = Backbone.View.extend({
    ...
    mapData : function(d){
      ...
      var parent       = {name : "Recursos de revisión"};

      parent.children = sectores.map(function(sec){
        return {
          name     : sec
        }
      });
      ...
      parent.children.forEach(function(ch){
        ...
        //childrens.childrens = _.where(d, {sector : ch.name, dependencia});

        ch.children = children;
      });

      return [{"mapadai" : parent.children}];
    },

    render : function(d, range){
      var r = this.mapData(d);
      /*
...
/* load in data, display root */
  function xxx(r) {
  root = {name : "recursos", children: r[0].mapadai};
